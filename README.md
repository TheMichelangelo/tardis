...
- відкриття уроку за кодом на головній сторінці, лише з вибраними вчителем вправами;
...
- кнопкою «Поділитися уроком» вибрати вправи, скопіювати код або посилання
  та переглянути результат як учень.

### Коди уроків

Код має **7 цифр**: `КННММММ`, де `К` — клас, `НН` — сталий номер уроку
від 01 до 99, `ММММ` — десяткове значення бітової маски від 0001 до 1023.
Для 10 вправ існує 1023 непорожні комбінації, тому три десяткові цифри для
маски (і шестизначний код) не можуть представити їх усі.

Молодший біт відповідає першій позиції масиву вправ, наступний — другій
і так далі. Наприклад, `5010005` відкриває урок 01 п'ятого класу з першою
і третьою вправами (`5 = 0000000101₂`), а `5011023` — з усіма наявними
вправами. Код із нульовою маскою, невідомим уроком або лише заглушками
не відкриває урок і показує повідомлення про помилку.

Посилання має вигляд `https://<сайт>/<базовий-шлях>/?lessonCode=5010005`.
Воно працює без сервера чи збереження вибору на пристрої вчителя, зокрема
після перезавантаження GitHub Pages. Для посилань із нативного застосунку
можна задати `--dart-define=STEM_PUBLIC_URL=https://<сайт>/<базовий-шлях>/`;
за замовчуванням використовується `https://themichelangelo.github.io/tardis/`.
Поширений урок відкривається в учнівському режимі навіть у вчителя.

Поле `number` у JSON — сталий номер уроку в межах класу; його не можна
повторно використовувати для іншого уроку або змінювати при перестановці
модулів. Переклади того самого уроку мають однакові номери й позиції вправ.
Якщо перекладу немає, код відкриває урок мовою наявного матеріалу.

Кожен урок має 10 позицій у масиві `exercises` (або старому `exersices`).
Порожні позиції позначені `"isPlaceholder": true` та приховані в учнівському
й учительському інтерфейсах, лічильниках і PDF. При додаванні матеріалу
замініть заглушку на вправу в тій самій позиції; порядок наявних вправ
потрібно зберігати, щоб раніше надіслані коди залишалися правильними.
Завантажувач автоматично доповнює старі локальні уроки заглушками, а понад
10 позицій відхиляє. Нова вправа в конструкторі пропозицій займає порожню
позицію під час додавання пропозиції до каталогу.
